...
class CheckServer
{
    ...
    public function check()
    {
        ...
        if ($socket === false) {
            throw new ConnectErrorException(socket_strerror(socket_last_error()));
        } else {
            // 尝试连接到 MySQL 服务器
            ...
            if ($result === false) {
                ...
                if ($response === false){
                    throw new ServerErrorException('The server response timed out or may not be a MySQL server.');
                }
                // 关闭 socket 连接
                socket_close($socket);
            }
